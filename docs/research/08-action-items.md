...
| 33 | E9 with the other encoders (MobileCLIP-B, DINOv2-S) — the public rows said the ImageNet trunk was the lever, not more views | `run.py --source folder --backbone …`, `research/report.py` (encoder column) | 05 P27, X15 | done 6 Sep 2026: MobileCLIP-B takes cartons from 45.8 % to 89.6 % top-1 and cuts the unknown-accept rate at 95 % TPR from 0.795 to 0.080; DINOv2-S matches the old trunk's top-1 while abstaining better, so the two properties come apart (claims X17–X19) |
| 35 | A knowledge base the next session can start from: `CLAUDE.md` router, `docs/kb/` (repo map, pipeline, contracts, surfaces, research, decisions, gotchas, runbooks, state, glossary), and a graphify call-and-import graph refreshed with `graphify update .` | `CLAUDE.md`, `docs/kb/`, `tools/check_kb.py`, `tests/test_kb_links.py` | user request, 6 Sep 2026 | done: 371 references checked in CI; graph is 1,542 nodes over 108 files, gitignored |
| 36 | The Cloudflare account id and account email were committed in `site/.wrangler/` | `.gitignore`, whole git history | found while indexing, 6 Sep 2026 | done: untracked, gitignored, then `git filter-repo` over every ref and force-pushed; GitHub serves the blob at no revision; release assets intact |
| 37 | **E1, the closed-set baseline, was cited in four places and implemented in none** — including in `NOTICE`, as the reason the AGPL YOLO weights are kept at all. Written: the version 1 detector and the proposed system scored over the same twelve labels, on the same held-out views, with a `catalogue_coverage` column carrying the argument the accuracy column will not | `research/experiments.py`, `research/run.py`, `research/report.py`, `recognition/proposer.py`, `research/dataset.py`, `paper/main.tex`, `tests/test_e1_closed_set.py` | user request, 6 Sep 2026 | done: runs on synthetic and refuses honestly without the weights; twelve real numbers still need the capture session |

| 38 | The till window was the largest untested surface in the tree — 1,121 lines whose decision surface was only smoke-imported.  Now covered behaviourally: the sellability gate and its legal hours, the staff confirmation path, the basket weight check including the case it cannot perform, checkout and its two refusals, disambiguation, the centre freeze, and the scan worker | `tests/test_main_window.py` | found while indexing, 6 Sep 2026 | done: 19 tests; suite 178 → 210 |

| 39 | The generator behind every number in the paper, the script that runs the capture session, and the two scale tools had no tests at all; the dashboard's seeded theme said `dark` while `docs/DESIGN.md` and the fallback both said light; `detection_confidence` was a version 3 leftover nothing read | `tests/test_report_tables.py`, `tests/test_capture_and_bench.py`, `server/services/database.py`, `data/products.json`, `docs/shots/README.md` | found while indexing, 6 Sep 2026 | done: 21 tests; screenshot manifest written |
