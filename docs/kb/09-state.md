...
| — | **The escalation threshold is a guess.**  `escalation_supervise_above_baht` defaults to 100 and no measurement supports it.  `research/experiments.py:e6_fusion` already sweeps the weight tolerance against false alarms; adding value at risk as a second axis would turn the operating point into a measured curve rather than a setting | `docs/kb/06-decisions.md` |
